...
SequelizeSlugify.prototype.slugifyModel = function (Model, slugOptions) {
     ...
     */
    var checkSlug = function (slug, instance = {}) {

        const where = {
            slug: slug,
        };

        if (instance['$modelOptions'].uniqueKeys.unique_slug_provider_id) {
            const uniqueFields = instance['$modelOptions'].uniqueKeys.unique_slug_provider_id.fields;

            if (uniqueFields || false){
                uniqueFields.forEach((field) => {
                    if (field !== 'slug'){
                        where[field] = instance[field];
                    }
                });
            }
        }
    ...
    var addSourceSuffix = function (instance, sourceFields, suffixFields) {
        return (function suffixHelper(instance, sourceFields, suffixFields, suffixCount) {
            ...
            return checkSlug(slug, instance).then(function (isUnique) {
                if (isUnique) {
                    return slug;
                }
     ...
     */
    var addNumericSuffix = function (slugValue, instance) {
        return (function suffixHelper(slug, count) {
            var suffixedSlug = slug + '-' + count;

            return checkSlug(suffixedSlug, instance).then(function (isUnique) {
                if (isUnique) {
                    return suffixedSlug;
                }
    ...
    var handleSlugify = function (instance, options, next) {
        ...
        return checkSlug(slugValue, instance).then(function (isUnique) {
            ...
            return checkSlug(slug, instance);
            ...
            return addNumericSuffix(slugValue, instance);
        }).then(function (slug) {
            // update the slug
            instance.slug = slug;
